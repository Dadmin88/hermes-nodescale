#!/usr/bin/env python3
"""Retained N4B disposable ingress-to-Headscale join proof."""
...
BASE_PREFIX = "nodescale-n4b-proof"
...
def resolve_exact_tree() -> str:
    requested = os.environ.get("NODESCALE_N4B_TREE")
    if not requested:
        raise RuntimeError("NODESCALE_N4B_TREE must name the immutable candidate tree")
    ...
    for relative in ["proofs/n4b/run.py", "proofs/n4b/images.lock"]:
...
def wait_proof_ready(path: Path, process: subprocess.Popen[bytes], log_path: Path) -> None:
    ...
    while not path.exists():
        if process.poll() is not None:
            ...
            raise RuntimeError(f"Rust proof exited before ingress readiness:\n{tail}")
        if time.monotonic() >= deadline:
            raise RuntimeError("timed out waiting for Rust ingress readiness")
        time.sleep(0.1)
...
def make_certificates(root: Path, gateway: str) -> None:
    ...
    run([
        ...
        "-subj", "/CN=Nodescale N4B Disposable CA",
    ])
    ...
    extensions.write_text(
        "subjectAltName=DNS:headscale,DNS:localhost,DNS:ingress.n4b.test,IP:127.0.0.1,IP:" + gateway + "\n"
...
derp:
  server:
    ...
    region_id: 999
    region_code: n4b
    region_name: Nodescale N4B Disposable DERP
    verify_clients: true
...
def build_proof_binary(root: Path, source: Path) -> Path:
    ...
    result = run(
        [
            "cargo", "test", "--locked", "-p", "nodescale-redemption-ingress",
            "--test", "disposable_join", "--no-run", "--message-format=json",
        ],
        env=env,
        cwd=source,
        check=False,
    ...
    for raw in result.stdout.splitlines():
        ...
        if (
            ...
            and target.get("name") == "disposable_join"
...
def start_rust_proof(
    ...
    environment = [
        "-e", "NODESCALE_N4B_PROOF_ROOT=/proof",
        "-e", "NODESCALE_N4B_PROOF_STATE_DB=/proof/state.db",
        "-e", "NODESCALE_N4B_HEADSCALE_URL=https://headscale:8080",
        "-e", "NODESCALE_N4B_LOGIN_SERVER=https://headscale:8080",
        "-e", f"NODESCALE_N4B_INGRESS_BIND=0.0.0.0:{INGRESS_PORT}",
        "-e", "NODESCALE_N4B_ALLOW_PUBLIC_BIND=proof-only",
        "-e", "NODESCALE_N4B_CA_FILE=/proof/ca.pem",
        "-e", "NODESCALE_N4B_INGRESS_CERT_FILE=/proof/tls.crt",
        "-e", "NODESCALE_N4B_INGRESS_KEY_FILE=/proof/tls.key",
        "-e", "NODESCALE_N4B_HEADSCALE_API_KEY_FILE=/proof/headscale-api-key",
    ]
    process = subprocess.Popen(
        [
            "docker", "run", "--platform", PLATFORM, "--rm", "--name", INGRESS,
            "--network", NETWORK, "--network-alias", "ingress.n4b.test",
            "--user", "1000:1000",
...
https://ingress.n4b.test:{INGRESS_PORT}/v1/redemptions'''
    process = subprocess.Popen(
        [
...
def start_tailscale(root: Path, image: str) -> None:
    ...
    docker(
        ...
        "-e", "SSL_CERT_FILE=/proof/ca.pem",
        "-e", f"TS_EXTRA_ARGS=--login-server={login_server} --accept-dns=false --hostname=nodescale-n4b-ephemeral",
        image,
    )
...
def main() -> int:
    ...
    try:
        ...
        root.chmod(0o700)
        print("N4B proof runtime initialized", file=sys.stderr, flush=True)
        ...
        wait_proof_ready(root / "ingress-ready", cargo, cargo_log)
        ...
        wait_file(root / "cleanup-complete")
...
if __name__ == "__main__":
    try:
        ...
        print(f"N4B proof failed: {error}", file=sys.stderr)
        raise SystemExit(1)
