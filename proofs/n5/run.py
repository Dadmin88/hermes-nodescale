...
def cleanup_resources(root: Path | None, cargo: subprocess.Popen[bytes] | None) -> list[str]:
    errors: list[str] = []
    for process in tuple(_ACTIVE_PROCESSES):
        if process.poll() is None:
            try:
...
def main() -> int:
    ...
    try:
        ...
        cleanup_errors = cleanup_resources(root, cargo)

    postflight_errors: list[str] = []
    try:
        conflicts_after = conflicting_resources()
        if conflicts_after:
            postflight_errors.append(f"proof resources remained after cleanup: {conflicts_after}")
    except Exception as error:  # preserve every independent postflight gate
        postflight_errors.append(f"runtime residue scan failed: {error}")
    try:
        repository_after = command_hash(
            ["git", "status", "--porcelain=v1", "-z", "--untracked-files=all"]
        )
        if repository_before != repository_after:
            postflight_errors.append("repository status changed during proof")
    except Exception as error:
        postflight_errors.append(f"repository postflight failed: {error}")
    try:
        if cargo_lock_before != sha256((REPO / "Cargo.lock").read_bytes()):
            postflight_errors.append("ambient Cargo.lock changed during proof")
    except Exception as error:
        postflight_errors.append(f"Cargo.lock postflight failed: {error}")
    try:
        time.sleep(1.0)
        after = host_invariant()
        if before != after:
            changed = sorted(key for key in before if before[key] != after[key])
            postflight_errors.append(f"sanitized host-network invariant changed: {changed}")
    except Exception as error:
        postflight_errors.append(f"host-network postflight failed: {error}")
    if cleanup_errors or postflight_errors:
        raise RuntimeError("; ".join([*cleanup_errors, *postflight_errors]))
