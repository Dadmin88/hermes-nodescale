-- Nodescale V9 typed provenance. StateStore owns BEGIN IMMEDIATE and marker-last commit.

PRAGMA defer_foreign_keys=ON;

CREATE TEMP TABLE stage_n5_device_identities AS SELECT * FROM n5_device_identities;

CREATE TEMP TABLE stage_n5_provider_bindings AS SELECT * FROM n5_provider_bindings;

CREATE TEMP TABLE stage_n6_binding_decisions AS SELECT * FROM n6_binding_decisions;

CREATE TEMP TABLE stage_n6_binding_records AS SELECT * FROM n6_binding_records;

CREATE TEMP TABLE stage_n6_binding_challenges AS SELECT * FROM n6_binding_challenges;

CREATE TEMP TABLE stage_n6_binding_authorizations AS SELECT * FROM n6_binding_authorizations;

CREATE TEMP TABLE stage_n6_challenge_reservations AS SELECT * FROM n6_challenge_reservations;

CREATE UNIQUE INDEX n9_join_sessions_exact_identity_parent ON join_sessions(join_session_id,device_id,network_id);

CREATE UNIQUE INDEX n9_n4_dispatch_exact_identity_parent ON n4_join_session_dispatches(join_session_id,network_id);

DROP TABLE n6_challenge_reservations;

DROP TABLE n6_binding_authorizations;

DROP TABLE n6_binding_challenges;

DROP TABLE n6_binding_records;

DROP TABLE n6_binding_decisions;

DROP TABLE n5_provider_bindings;

DROP TABLE n5_device_identities;

CREATE TABLE n5_device_identities(device_id TEXT PRIMARY KEY REFERENCES devices(device_id) ON DELETE RESTRICT,network_id TEXT NOT NULL REFERENCES networks(network_id) ON DELETE RESTRICT,identity_origin_kind TEXT NOT NULL CHECK(identity_origin_kind IN ('n4_join_session','existing_provider_adoption')),identity_origin_id TEXT NOT NULL,n4_origin_id TEXT UNIQUE,adoption_origin_id TEXT UNIQUE,confirmed_at_ms INTEGER NOT NULL CHECK(typeof(confirmed_at_ms)='integer' AND confirmed_at_ms>=0),identity_revision INTEGER NOT NULL CHECK(typeof(identity_revision)='integer' AND identity_revision=1),safe_correlation_digest TEXT NOT NULL CHECK(length(safe_correlation_digest)=71 AND safe_correlation_digest GLOB 'sha256:*' AND substr(safe_correlation_digest,8) NOT GLOB '*[^0-9a-f]*'),UNIQUE(device_id,network_id),UNIQUE(device_id,network_id,identity_origin_kind,identity_origin_id),UNIQUE(identity_origin_kind,identity_origin_id),UNIQUE(identity_origin_id,device_id,network_id,identity_origin_kind),FOREIGN KEY(n4_origin_id,device_id,network_id,identity_origin_kind) REFERENCES n5_n4_identity_origins(origin_id,device_id,network_id,origin_kind) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED,FOREIGN KEY(adoption_origin_id,device_id,network_id,identity_origin_kind) REFERENCES n5_existing_adoption_identity_origins(origin_id,device_id,network_id,origin_kind) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED,CHECK((identity_origin_kind='n4_join_session' AND n4_origin_id=identity_origin_id AND adoption_origin_id IS NULL) OR (identity_origin_kind='existing_provider_adoption' AND adoption_origin_id=identity_origin_id AND n4_origin_id IS NULL)),CHECK(length(device_id)=36 AND device_id=lower(device_id) AND substr(device_id,9,1)='-' AND substr(device_id,14,1)='-' AND substr(device_id,19,1)='-' AND substr(device_id,24,1)='-' AND replace(device_id,'-','') NOT GLOB '*[^0-9a-f]*' AND device_id<>'00000000-0000-0000-0000-000000000000' AND length(network_id)=36 AND network_id=lower(network_id) AND substr(network_id,9,1)='-' AND substr(network_id,14,1)='-' AND substr(network_id,19,1)='-' AND substr(network_id,24,1)='-' AND replace(network_id,'-','') NOT GLOB '*[^0-9a-f]*' AND network_id<>'00000000-0000-0000-0000-000000000000' AND length(identity_origin_id)=36 AND identity_origin_id=lower(identity_origin_id) AND substr(identity_origin_id,9,1)='-' AND substr(identity_origin_id,14,1)='-' AND substr(identity_origin_id,19,1)='-' AND substr(identity_origin_id,24,1)='-' AND replace(identity_origin_id,'-','') NOT GLOB '*[^0-9a-f]*' AND identity_origin_id<>'00000000-0000-0000-0000-000000000000'));

CREATE TABLE n5_n4_identity_origins(origin_id TEXT PRIMARY KEY,origin_kind TEXT NOT NULL CHECK(origin_kind='n4_join_session'),device_id TEXT NOT NULL,network_id TEXT NOT NULL,join_session_id TEXT NOT NULL UNIQUE,UNIQUE(origin_id,device_id,network_id,origin_kind),UNIQUE(join_session_id,device_id,network_id),CHECK(origin_id=join_session_id),FOREIGN KEY(origin_id,device_id,network_id,origin_kind) REFERENCES n5_device_identities(identity_origin_id,device_id,network_id,identity_origin_kind) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED,FOREIGN KEY(join_session_id,device_id,network_id) REFERENCES join_sessions(join_session_id,device_id,network_id) ON DELETE RESTRICT ON UPDATE RESTRICT,FOREIGN KEY(join_session_id,network_id) REFERENCES n4_join_session_dispatches(join_session_id,network_id) ON DELETE RESTRICT ON UPDATE RESTRICT);

CREATE TABLE n5_existing_adoption_identity_origins(origin_id TEXT PRIMARY KEY,origin_kind TEXT NOT NULL CHECK(origin_kind='existing_provider_adoption'),device_id TEXT NOT NULL UNIQUE,network_id TEXT NOT NULL,action_id TEXT NOT NULL UNIQUE,proof_operation_id TEXT NOT NULL,evidence_id TEXT NOT NULL UNIQUE,decision_id TEXT NOT NULL UNIQUE,provider_binding_id TEXT NOT NULL UNIQUE,provider_instance_id TEXT NOT NULL,provider_node_id TEXT NOT NULL,observation_generation INTEGER NOT NULL,proof_generation INTEGER NOT NULL,machine_key_fingerprint TEXT NOT NULL,node_key_fingerprint TEXT NOT NULL,UNIQUE(origin_id,device_id,network_id,origin_kind),FOREIGN KEY(origin_id,device_id,network_id,origin_kind) REFERENCES n5_device_identities(identity_origin_id,device_id,network_id,identity_origin_kind) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED);
...
CREATE TABLE n5_provider_bindings (
    ...
    provenance_kind TEXT NOT NULL CHECK (provenance_kind IN ('n4_join_session','existing_provider_adoption')),
    n4_provenance_binding_id TEXT UNIQUE,
    adoption_provenance_binding_id TEXT UNIQUE,
...
,
 UNIQUE(device_id,network_id),UNIQUE(binding_id,device_id,network_id),UNIQUE(binding_id,device_id,network_id,provenance_kind,provider_instance_id),UNIQUE(binding_id,device_id,network_id,provenance_kind,provider_instance_id,provider_node_id,machine_key_fingerprint),FOREIGN KEY(device_id,network_id) REFERENCES n5_device_identities(device_id,network_id) ON DELETE RESTRICT ON UPDATE RESTRICT,FOREIGN KEY(n4_provenance_binding_id,device_id,network_id,provenance_kind,provider_instance_id) REFERENCES n5_n4_provider_binding_provenance(binding_id,device_id,network_id,provenance_kind,provider_instance_id) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED,FOREIGN KEY(adoption_provenance_binding_id,device_id,network_id,provenance_kind,provider_instance_id,provider_node_id,machine_key_fingerprint) REFERENCES n5_existing_adoption_provider_binding_provenance(binding_id,device_id,network_id,provenance_kind,provider_instance_id,provider_node_id,machine_key_fingerprint) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED,CHECK((provenance_kind='n4_join_session' AND n4_provenance_binding_id=binding_id AND adoption_provenance_binding_id IS NULL) OR (provenance_kind='existing_provider_adoption' AND adoption_provenance_binding_id=binding_id AND n4_provenance_binding_id IS NULL)));

CREATE TABLE n5_n4_provider_binding_provenance(binding_id TEXT PRIMARY KEY,provenance_kind TEXT NOT NULL CHECK(provenance_kind='n4_join_session'),device_id TEXT NOT NULL,network_id TEXT NOT NULL,identity_origin_kind TEXT NOT NULL CHECK(identity_origin_kind='n4_join_session'),identity_origin_id TEXT NOT NULL,join_session_id TEXT NOT NULL UNIQUE,credential_id TEXT NOT NULL,provider_credential_reference TEXT NOT NULL CHECK(length(provider_credential_reference) BETWEEN 1 AND 255),provider_instance_id TEXT NOT NULL,UNIQUE(binding_id,device_id,network_id,provenance_kind,provider_instance_id),CHECK(join_session_id=identity_origin_id),FOREIGN KEY(binding_id,device_id,network_id,provenance_kind,provider_instance_id) REFERENCES n5_provider_bindings(binding_id,device_id,network_id,provenance_kind,provider_instance_id) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED,FOREIGN KEY(identity_origin_id,device_id,network_id,identity_origin_kind) REFERENCES n5_n4_identity_origins(origin_id,device_id,network_id,origin_kind) ON DELETE RESTRICT ON UPDATE RESTRICT,FOREIGN KEY(join_session_id,credential_id,network_id,provider_instance_id) REFERENCES n4_provider_credential_metadata(join_session_id,credential_id,network_id,provider_instance_id) ON DELETE RESTRICT ON UPDATE RESTRICT,FOREIGN KEY(credential_id,network_id,provider_instance_id,provider_credential_reference) REFERENCES confirmed_provider_credential_references(credential_id,network_id,provider_instance_id,provider_reference) ON DELETE RESTRICT ON UPDATE RESTRICT);

CREATE TABLE n5_existing_adoption_provider_binding_provenance(binding_id TEXT PRIMARY KEY,provenance_kind TEXT NOT NULL CHECK(provenance_kind='existing_provider_adoption'),device_id TEXT NOT NULL,network_id TEXT NOT NULL,identity_origin_kind TEXT NOT NULL CHECK(identity_origin_kind='existing_provider_adoption'),identity_origin_id TEXT NOT NULL,action_id TEXT NOT NULL UNIQUE,proof_operation_id TEXT NOT NULL,evidence_id TEXT NOT NULL UNIQUE,decision_id TEXT NOT NULL UNIQUE,provider_instance_id TEXT NOT NULL,provider_node_id TEXT NOT NULL,observation_generation INTEGER NOT NULL,proof_generation INTEGER NOT NULL,machine_key_fingerprint TEXT NOT NULL,node_key_fingerprint TEXT NOT NULL,UNIQUE(binding_id,device_id,network_id,provenance_kind,provider_instance_id,provider_node_id,machine_key_fingerprint),FOREIGN KEY(binding_id,device_id,network_id,provenance_kind,provider_instance_id,provider_node_id,machine_key_fingerprint) REFERENCES n5_provider_bindings(binding_id,device_id,network_id,provenance_kind,provider_instance_id,provider_node_id,machine_key_fingerprint) ON DELETE RESTRICT ON UPDATE RESTRICT DEFERRABLE INITIALLY DEFERRED,FOREIGN KEY(identity_origin_id,device_id,network_id,identity_origin_kind) REFERENCES n5_existing_adoption_identity_origins(origin_id,device_id,network_id,origin_kind) ON DELETE RESTRICT ON UPDATE RESTRICT);
...
CREATE TABLE n6_binding_records (
    ...
    device_id TEXT NOT NULL REFERENCES n5_device_identities(device_id) ON DELETE RESTRICT ON UPDATE RESTRICT
        CHECK (length(device_id) BETWEEN 1 AND 64 AND device_id NOT GLOB '*[^A-Za-z0-9_.:-]*'),
    n5_provider_binding_id TEXT NOT NULL CHECK (length(n5_provider_binding_id) BETWEEN 1 AND 64 AND n5_provider_binding_id NOT GLOB '*[^A-Za-z0-9_.:-]*'),
...
,
    CHECK ((length(challenge_id) = 36 AND challenge_id = lower(challenge_id) AND substr(challenge_id,9,1) = '-' AND substr(challenge_id,14,1) = '-' AND substr(challenge_id,19,1) = '-' AND substr(challenge_id,24,1) = '-' AND replace(challenge_id,'-','') NOT GLOB '*[^0-9a-f]*' AND challenge_id <> '00000000-0000-0000-0000-000000000000') AND (length(binding_id) = 36 AND binding_id = lower(binding_id) AND substr(binding_id,9,1) = '-' AND substr(binding_id,14,1) = '-' AND substr(binding_id,19,1) = '-' AND substr(binding_id,24,1) = '-' AND replace(binding_id,'-','') NOT GLOB '*[^0-9a-f]*' AND binding_id <> '00000000-0000-0000-0000-000000000000') AND (length(network_id) = 36 AND network_id = lower(network_id) AND substr(network_id,9,1) = '-' AND substr(network_id,14,1) = '-' AND substr(network_id,19,1) = '-' AND substr(network_id,24,1) = '-' AND replace(network_id,'-','') NOT GLOB '*[^0-9a-f]*' AND network_id <> '00000000-0000-0000-0000-000000000000') AND (length(device_id) = 36 AND device_id = lower(device_id) AND substr(device_id,9,1) = '-' AND substr(device_id,14,1) = '-' AND substr(device_id,19,1) = '-' AND substr(device_id,24,1) = '-' AND replace(device_id,'-','') NOT GLOB '*[^0-9a-f]*' AND device_id <> '00000000-0000-0000-0000-000000000000') AND (length(binding_id) = 36 AND binding_id = lower(binding_id) AND substr(binding_id,9,1) = '-' AND substr(binding_id,14,1) = '-' AND substr(binding_id,19,1) = '-' AND substr(binding_id,24,1) = '-' AND replace(binding_id,'-','') NOT GLOB '*[^0-9a-f]*' AND binding_id <> '00000000-0000-0000-0000-000000000000') AND (length(last_decision_id) = 36 AND last_decision_id = lower(last_decision_id) AND substr(last_decision_id,9,1) = '-' AND substr(last_decision_id,14,1) = '-' AND substr(last_decision_id,19,1) = '-' AND substr(last_decision_id,24,1) = '-' AND replace(last_decision_id,'-','') NOT GLOB '*[^0-9a-f]*' AND last_decision_id <> '00000000-0000-0000-0000-000000000000') AND (length(last_audit_event_id) = 36 AND last_audit_event_id = lower(last_audit_event_id) AND substr(last_audit_event_id,9,1) = '-' AND substr(last_audit_event_id,14,1) = '-' AND substr(last_audit_event_id,19,1) = '-' AND substr(last_audit_event_id,24,1) = '-' AND replace(last_audit_event_id,'-','') NOT GLOB '*[^0-9a-f]*' AND last_audit_event_id <> '00000000-0000-0000-0000-000000000000')),
 FOREIGN KEY(binding_id,network_id,device_id,generation) REFERENCES n6_binding_records(binding_id,network_id,device_id,generation) ON DELETE RESTRICT ON UPDATE RESTRICT);
...
INSERT INTO n5_device_identities(device_id,network_id,identity_origin_kind,identity_origin_id,n4_origin_id,adoption_origin_id,confirmed_at_ms,identity_revision,safe_correlation_digest) SELECT device_id,network_id,'n4_join_session',origin_join_session_id,origin_join_session_id,NULL,confirmed_at_ms,identity_revision,safe_correlation_digest FROM temp.stage_n5_device_identities;

INSERT INTO n5_n4_identity_origins(origin_id,origin_kind,device_id,network_id,join_session_id) SELECT origin_join_session_id,'n4_join_session',device_id,network_id,origin_join_session_id FROM temp.stage_n5_device_identities;

INSERT INTO n5_provider_bindings(binding_id,device_id,network_id,provider_instance_id,provider_node_id,machine_key_fingerprint,binding_state,binding_revision,observed_at_ms,stale_at_ms,cleanup_pending_at_ms,removed_at_ms,last_transition_audit_event_id,transition_actor_source,transition_actor_id,provenance_kind,n4_provenance_binding_id,adoption_provenance_binding_id) SELECT binding_id,device_id,network_id,provider_instance_id,provider_node_id,machine_key_fingerprint,binding_state,binding_revision,observed_at_ms,stale_at_ms,cleanup_pending_at_ms,removed_at_ms,last_transition_audit_event_id,transition_actor_source,transition_actor_id,'n4_join_session',binding_id,NULL FROM temp.stage_n5_provider_bindings;

INSERT INTO n5_n4_provider_binding_provenance(binding_id,provenance_kind,device_id,network_id,identity_origin_kind,identity_origin_id,join_session_id,credential_id,provider_credential_reference,provider_instance_id) SELECT binding_id,'n4_join_session',device_id,network_id,'n4_join_session',join_session_id,join_session_id,credential_id,provider_credential_reference,provider_instance_id FROM temp.stage_n5_provider_bindings;

INSERT INTO n6_binding_records(binding_id,network_id,device_id,verified_peer_id,generation,revision,binding_state,created_at_ms,confirmed_at_ms,stale_at_ms,rotated_at_ms,revoked_at_ms,last_verified_at_ms,rotated_from_binding_id,rotation_authorization_id,agent_version,last_decision_id,last_audit_event_id,n5_provider_binding_id) SELECT r.binding_id,r.network_id,r.device_id,r.verified_peer_id,r.generation,r.revision,r.binding_state,r.created_at_ms,r.confirmed_at_ms,r.stale_at_ms,r.rotated_at_ms,r.revoked_at_ms,r.last_verified_at_ms,r.rotated_from_binding_id,r.rotation_authorization_id,r.agent_version,r.last_decision_id,r.last_audit_event_id,p.binding_id FROM temp.stage_n6_binding_records r JOIN temp.stage_n5_provider_bindings p ON p.join_session_id=r.join_session_id AND p.device_id=r.device_id AND p.network_id=r.network_id;

INSERT INTO n6_binding_decisions(decision_id,audit_event_id,subject_kind,decision_kind,binding_id,challenge_id,authorization_id,network_id,device_id,generation,prior_state,new_state,prior_revision,new_revision,decided_at_ms,actor_source,actor_id,reason_code,authenticated_peer_id,operation_id,agent_version) SELECT decision_id,audit_event_id,subject_kind,decision_kind,binding_id,challenge_id,authorization_id,network_id,device_id,generation,prior_state,new_state,prior_revision,new_revision,decided_at_ms,actor_source,actor_id,reason_code,authenticated_peer_id,operation_id,agent_version FROM temp.stage_n6_binding_decisions;

INSERT INTO n6_binding_challenges(challenge_id,binding_id,network_id,device_id,expected_authenticated_peer_id,generation,challenge_verifier,challenge_state,issued_at_ms,expires_at_ms,consumed_at_ms,invalidated_at_ms,expired_at_ms,consumed_operation_id,consumed_authenticated_peer_id,agent_version,last_decision_id,last_audit_event_id) SELECT challenge_id,binding_id,network_id,device_id,expected_authenticated_peer_id,generation,challenge_verifier,challenge_state,issued_at_ms,expires_at_ms,consumed_at_ms,invalidated_at_ms,expired_at_ms,consumed_operation_id,consumed_authenticated_peer_id,agent_version,last_decision_id,last_audit_event_id FROM temp.stage_n6_binding_challenges;

INSERT INTO n6_binding_authorizations(authorization_id,authority_id,binding_id,network_id,device_id,generation,expected_revision,action_kind,actor_source,actor_id,issued_at_ms,expires_at_ms,issued_decision_id,issued_audit_event_id,authorization_state,consumed_at_ms,expired_at_ms,invalidated_at_ms,consumed_decision_id,consumed_audit_event_id,expired_decision_id,expired_audit_event_id,invalidated_decision_id,invalidated_audit_event_id) SELECT authorization_id,authority_id,binding_id,network_id,device_id,generation,expected_revision,action_kind,actor_source,actor_id,issued_at_ms,expires_at_ms,issued_decision_id,issued_audit_event_id,authorization_state,consumed_at_ms,expired_at_ms,invalidated_at_ms,consumed_decision_id,consumed_audit_event_id,expired_decision_id,expired_audit_event_id,invalidated_decision_id,invalidated_audit_event_id FROM temp.stage_n6_binding_authorizations;

INSERT INTO n6_challenge_reservations(reservation_id,binding_id,network_id,device_id,expected_authenticated_peer_id,operation_id,request_fingerprint,generation,expires_at_ms,agent_version,reservation_state,reserved_at_ms,issued_at_ms,abandoned_at_ms,challenge_id) SELECT reservation_id,binding_id,network_id,device_id,expected_authenticated_peer_id,operation_id,request_fingerprint,generation,expires_at_ms,agent_version,reservation_state,reserved_at_ms,issued_at_ms,abandoned_at_ms,challenge_id FROM temp.stage_n6_challenge_reservations;
...
CREATE TRIGGER n5_existing_adoption_identity_origin_v9_block BEFORE INSERT ON n5_existing_adoption_identity_origins BEGIN SELECT RAISE(ABORT,'V9 does not create adopted identities'); END;

CREATE TRIGGER n5_existing_adoption_provider_binding_v9_block BEFORE INSERT ON n5_existing_adoption_provider_binding_provenance BEGIN SELECT RAISE(ABORT,'V9 does not create adoption provider bindings'); END;
...
FOR EACH ROW WHEN NEW.binding_state <> 'pending'
    OR NEW.revision <> 1
    OR NOT EXISTS (
        SELECT 1 FROM n5_provider_bindings i
        JOIN n6_binding_decisions d ON d.decision_id = NEW.last_decision_id
        WHERE i.binding_id = NEW.n5_provider_binding_id
...
BEGIN SELECT RAISE(ABORT, 'N6 decision requires exact N5 identity, semantic audit, and public metadata'); END;
...
FOR EACH ROW BEGIN SELECT RAISE(ABORT, 'N5 device identity is immutable'); END;
CREATE TRIGGER n5_n4_identity_origin_immutable
BEFORE UPDATE ON n5_n4_identity_origins
FOR EACH ROW BEGIN SELECT RAISE(ABORT, 'N5 N4 identity origin is immutable'); END;
CREATE TRIGGER n5_n4_identity_origin_immutable_delete
BEFORE DELETE ON n5_n4_identity_origins
FOR EACH ROW BEGIN SELECT RAISE(ABORT, 'N5 N4 identity origin is immutable'); END;
...
CREATE TRIGGER n5_n4_provider_binding_provenance_immutable
BEFORE UPDATE ON n5_n4_provider_binding_provenance
FOR EACH ROW BEGIN SELECT RAISE(ABORT, 'N5 N4 provider binding provenance is immutable'); END;
CREATE TRIGGER n5_n4_provider_binding_provenance_immutable_delete
BEFORE DELETE ON n5_n4_provider_binding_provenance
FOR EACH ROW BEGIN SELECT RAISE(ABORT, 'N5 N4 provider binding provenance is immutable'); END;
