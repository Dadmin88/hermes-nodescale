...
CREATE TABLE n5_trust_decisions (
    ...
    authority_id TEXT NOT NULL REFERENCES n5_trust_authorities(authority_id) ON DELETE RESTRICT,
    authority_generation INTEGER NOT NULL CHECK (typeof(authority_generation) = 'integer' AND authority_generation >= 1),
    authorized_principal_source TEXT NOT NULL CHECK (length(authorized_principal_source) BETWEEN 1 AND 64),
    authorized_principal_id TEXT NOT NULL CHECK (length(authorized_principal_id) BETWEEN 1 AND 255),
...
BEFORE UPDATE OF binding_state, binding_revision, stale_at_ms, cleanup_pending_at_ms, removed_at_ms
ON n5_provider_bindings
FOR EACH ROW WHEN NEW.binding_revision <> OLD.binding_revision + 1 OR NOT (
