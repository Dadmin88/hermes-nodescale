...
use nodescale_domain::{
    ...
    NetworkId, OwnerTrustRootToken, ProviderCredentialId, ProviderCredentialReference,
    ProviderIdentity, ProviderInstanceId, ProviderKind, ProviderNodeId, Role, Roles,
...
async fn provider_reconciliation_preserves_exact_binding_and_stales_absence() {
    ...
    let exact = store
        ...
        .await
        .unwrap();
    assert!(exact.active_binding.is_some());
    ...
    let stale = store
        ...
        .await
        .unwrap();
    assert!(stale.active_binding.is_none());
...
async fn identity_is_exact_untrusted_then_explicitly_trusted_and_terminally_revoked() {
    ...
    let confirmation = store
        .confirm_n5_device_identity(&provider, join_session_id, now(), AuditActor::system())
        .await
        .unwrap();
    assert_eq!(
    ...
    let before = store
        .device_trust_view(confirmation.identity.device_id)
        .unwrap();
    assert_eq!(before.trust_state, DeviceTrustState::Untrusted);
    assert!(!before.currently_trusted);
    assert_eq!(store.trusted_device_count(network.network_id).unwrap(), 0);
    ...
    assert_eq!(store.trusted_device_count(network.network_id).unwrap(), 1);
    ...
    assert!(removed.active_binding.is_none());
    ...
    let activation_after_revocation = store
        .issue_device_trust_authorization(
            &root_token,
            authority_id,
            confirmation.identity.device_id,
...
async fn independent_connections_race_one_identity_and_one_trust_revision() {
        ...
        .map(|_| {
            ...
            thread::spawn(move || {
                ...
                let runtime = tokio::runtime::Builder::new_current_thread()
                    ...
                    .unwrap();
                barrier.wait();
                runtime.block_on(store.confirm_n5_device_identity(
                    &provider,
                    join_session_id,
                    now(),
                    AuditActor::system(),
    ...
    let handles = [first, second].map(|authorization| {
        let path = path.clone();
