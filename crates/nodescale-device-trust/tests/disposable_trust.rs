...
use nodescale_state::{
    HeadscaleImportConfig, N5TrustAuthorityConfiguration, N5TrustReason,
...
async fn disposable_join_confirms_identity_activates_revokes_and_cleans_up() {
    ...
    let trust_service = DeviceIdentityService::new(&identity_store, &read_provider);
    assert_eq!(
        trust_service
            .trusted_device_count(network.network_id)
            .unwrap(),
        0
    );
    ...
    let before = trust_service.trust_view(device_id).unwrap();
    ...
    assert!(read_provider.list_nodes().await.unwrap().is_empty());
    let cleanup_trust = DeviceIdentityService::new(&cleanup_store, &read_provider);
