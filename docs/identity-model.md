...
2. **Nodescale device identity** — a Nodescale-generated opaque UUID durably correlated from one confirmed N4 join session to one exact provider registration; it is not a hostname, address, provider numeric ID, or trust decision.
...
## N5 logical identity and trust boundary

N5 creates a logical `DeviceId` only after an exact, unique Headscale node pre-auth reference matches the confirmed provider-native credential reference for one N4 join session and its machine-key evidence matches the adapter fingerprint. The full contract, lifecycle, typed authority, audit history, replacement policy, and N5→N6 boundary are defined in [device-trust.md](device-trust.md).

The logical device, provider binding, and trust decision have separate lifetimes. Every confirmed identity starts untrusted. Trust activation and revocation consume explicit durable authorization actions; roles and membership grant nothing. A trusted logical state with a stale provider binding fails the authoritative current-trust query closed. N5 does not create or verify Keryx identity.
